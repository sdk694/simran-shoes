const HeroComponent = (() => {
  const FALLBACK = [
    ...
    },
    {
      ...
      sub: "Sandals, chappals and open-toe freedom — crafted for Madhya Pradesh summers.",
      img: 'https://images.unsplash.com/photo-1515955656352-a1fa3ffcd111?w=1600&q=80&auto=format&fit=crop',
    ...
    },
    {
      ...
      sub: "Authentic traditional footwear for every occasion — from pujas to pandals.",
      img: 'https://images.unsplash.com/photo-1600185365926-3a2ce3cdb9eb?w=1600&q=80&auto=format&fit=crop',
  ...
  function getShell() {
    return `
      <section class="hero" id="home" aria-label="Hero campaign">
        <div class="hero-img-bg" id="hero-img-bg">
          <img id="hero-bg-img" src="" alt="" aria-hidden="true" />
        </div>
  ...
  function renderCampaign(c) {
    ...
    content.innerHTML = `
      ...
      <div class="hero-cta-group">
        ...
        ${c.ctaSecondary?.label ? `<a href="${sanitize(c.ctaSecondary.target || '#')}" class="btn btn-outline btn-lg">${sanitize(c.ctaSecondary.label)}</a>` : ''}
      </div>
  ...
  function goTo(i) {
    _idx = (i + _campaigns.length) % _campaigns.length;
    renderCampaign(_campaigns[_idx]);
    renderDots();
  }

  function startAuto() {
    if (_campaigns.length <= 1) return;
    _timer = setInterval(() => goTo(_idx + 1), 6500);
  }
